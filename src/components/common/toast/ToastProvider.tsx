...
const DEFAULT_TOAST_MESSAGE = "功能开发中，敬请期待";
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const showToast = useCallback(
    (message?: string) => {
      const text = message?.trim() || DEFAULT_TOAST_MESSAGE;
  ...
  return (
    <ToastContext.Provider value={contextValue}>
      ...
      <div className="pointer-events-none fixed inset-x-0 top-8 z-[1200] flex justify-center px-4">
        <div
          role="status"
          aria-live="polite"
          aria-atomic="true"
          className={`pointer-events-auto flex max-w-[420px] items-center gap-3 rounded-2xl border border-white/15 bg-[#1f1f1f]/95 px-5 py-3 text-sm text-white shadow-[0_18px_38px_rgba(0,0,0,0.45)] backdrop-blur-md transition-all duration-200 ${toastState.visible ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
            }`}
        >
